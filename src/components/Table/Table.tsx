...
type Props = {
  ...
  setSpaId: Dispatch<SetStateAction<string | null>>;
  toggleRatingModal: () => void;
};

const Table: FC<Props> = ({ bookingDetails, setSpaId, toggleRatingModal }) => {
    ...
    <div className='overflow-x-auto max-w-[340px] rounded-lg mx-auto md:max-w-full shadow-md sm:rounded-lg'>
      <table className='w-full text-sm text-left text-gray-500'>
        <thead className='text-xs text-gray-700 uppercase bg-gray-50'>
          <tr>
            <th className='px-6 py-3'>Spa name</th>
        ...
        <tbody>
          {bookingDetails.map(booking => (
            ...
            >
              <th
                onClick={() =>
                  router.push(`/spas/${booking.goldenView.slug.current}`)
                }
                className='px-6 underline text-blue-600 cursor-pointer py-4 font-medium whitespace-nowrap'
              >
                {booking.goldenView.name}
              </th>
              <td className='px-6 py-4'>{booking.goldenView.price}</td>
              ...
              <td className='px-6 py-4'>
                <button
                  onClick={() => {
                    setSpaId(booking.goldenView._id);
                    toggleRatingModal()
                  }}
